...
def blit_all(food_pos):
    ...
    global game
    tupnumapl = number_on_apple(food_pos)
    list_of_tuples = []
    # Creates the number on the apple
    tup_score = (write(f"Score: {Costants.score}", 0, 0), (0, 0))
    b_maxiscore = write(f"Max-score: {game.maxscore}", 0, 0)
    tup_max = (b_maxiscore, (300, 0))
    # To obscure the text for score and maxscore
    tup_bscore1 = (Costants.bscore1, (0, 0))
    tup_bscore2 = (Costants.bscore2, (300, 0))

    tup_fruit = (Costants.fruit, (food_pos[0] * Costants.BLOCK_SIZE, food_pos[1] * Costants.BLOCK_SIZE))
    ...
    for surface in (tup_fruit, tupnumapl, tup_bscore1, tup_bscore2, tup_score, tup_max):
        list_of_tuples.append(surface)
...
def build_snake(list_of_sprites, _snake):
    ...
    for n, pos in enumerate(_snake):
        ...
        if n == 0:
            bbody = (Costants.head, (pos[0] * Costants.BLOCK_SIZE, pos[1] * Costants.BLOCK_SIZE))
        else:
